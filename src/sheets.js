...
export function findPlannerLayout(rows) {
	const headerRow = rows.findIndex((r) => r.filter((c) => /-FEIRA|SÁBADO/i.test(c)).length >= 3);
	if (headerRow === -1) {
		throw new Error("planejamento: linha dos dias da semana não encontrada");
	}

	const timeRow = rows.findIndex((r, i) => i > headerRow && r.some((c) => normalize(c) === "DATA/HR"));
	if (timeRow === -1) {
	...
	rows[headerRow].forEach((label, col) => {
		...
		for (let j = col; j < rows[timeRow].length; j++) {
			if (normalize(cell(rows, timeRow, j)) === "DATA/HR") {
				dateCol = j;
				break;
			}
		}
		...
		for (let j = dateCol + 1; j < rows[timeRow].length; j++) {
			...
			if (!isTime(value)) {
				throw new Error(`planejamento: célula inesperada '${value}' na linha de horários de ${label.trim()}`);
			}
...
export function parseLegend(rows, headerRow) {
	...
	for (let r = 0; r < headerRow && legendCol === -1; r++) {
		for (let c = 0; c < (rows[r]?.length ?? 0); c++) {
			if (normalize(cell(rows, r, c)) === "LEGENDA") {
				legendCol = c;
				break;
			}
		}
...
export function parsePlanner(csvText, { holidayCode = "Fer/Rec", periodMinutes = 50 } = {}) {
	...
	for (let r = layout.dataStartRow; r < rows.length; r++) {
		for (const day of layout.days) {
			...
			if (/^[1-7]$/.test(marker) && Number(marker) !== day.weekday) {
				throw new Error(`planejamento: ${day.label} (${day.weekday}) traz marcador ${marker} em ${raw}`);
			}
	...
	for (const day of days) {
		for (const block of day.blocks) {
			periodCounts.set(block.subject, (periodCounts.get(block.subject) ?? 0) + block.slots.length);
		}
	}
...
export function parseGrade(csvText) {
	...
	const column = columnsOf(rows[headerRow], ["DIA DA SEMANA", "HORARIO", "DISCIPLINA", "LOCAL"]);
	...
	for (let r = headerRow + 1; r < rows.length; r++) {
		...
		if (/^[1-7]$/.test(marker)) {
			if (weekday === null) weekday = Number(marker);
				...
				throw new Error(`grade: linha ${r + 1} diz ${marker} mas o cabeçalho do grupo diz ${weekday}`);
			}
		}
...
export function parseSubjects(csvText, phase) {
	...
	const column = columnsOf(rows[headerRow], ["SIGLA", "FASE", "COD SIGAA", "DISCIPLINA", "CH 50 MIN", "PROFESSOR"]);
		...
		days[day.date] = {
			...
			blocks: day.blocks.map((block) => {
				...
				return {
					...
					teacher: (matches ? slotMeta.teacher : null) ?? subjects[block.subject]?.teacher ?? null,
				};
			}),
		};
	}
