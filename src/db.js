...
const PUBLIC_COLUMNS = "id, name, color, photo_version, (photo IS NOT NULL) AS has_photo";
...
export function createUser(db, { name, color, pinHash, pinSalt, photo, photoType }) {
  ...
  `,
		...
		.run(name.trim(), key, color, photo ?? null, photoType ?? null, photo ? 1 : 0, pinHash, pinSalt, now());
...
export function listUsers(db) {
	return db.prepare(`SELECT ${PUBLIC_COLUMNS} FROM users ORDER BY name_key`).all().map(toPublic);
}
...
export function createSession(db, tokenHash, userId) {
	db.prepare("INSERT INTO sessions (token_hash, user_id, created_at, last_seen) VALUES (?, ?, ?, ?)").run(
		tokenHash,
		userId,
		now(),
		now(),
...
export function setReason(db, { userId, date, slots, reason }) {
	const update = db.prepare("UPDATE absences SET reason = ? WHERE user_id = ? AND date = ? AND slot = ?");
	let changed = 0;
	db.exec("BEGIN");
	try {
