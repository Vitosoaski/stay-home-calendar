...
export function renderPanels(state, { users }) {
	...
	return el("aside", { class: "paineis" }, todayPanel(state, byId), rankingPanel(state, byId), frequencyPanel(state));
}
...
function todayPanel(state, byId) {
	...
	return panel("Hoje", rows, state.today.blocks.length ? "Ninguém marcou falta hoje." : "Hoje não tem aula.");
}
	...
	const rows = state.days
		...
		.map(({ day, block }) =>
			el(
				...
				{ class: "linha" },
				el("span", { class: "sigla-pequena" }, block.subject),
				el("span", { class: "hora-pequena" }, `${DIA_CURTO[day.weekday] ?? ""} ${block.start}`),
				faces(block.absences, byId),
			),
		);
...
function frequencyPanel(state) {
	...
	const rows = state.frequency
		...
		.map((item) => {
			...
			const level = item.remaining <= 0 ? "estourado" : item.remaining <= 2 ? "atencao" : "ok";

			return el(
				"div",
				{ class: `frequencia ${level}` },
				el(
					"div",
					{ class: "linha" },
					...
					el(
						"span",
						{ class: "restante" },
						item.remaining > 0 ? `pode faltar mais ${item.remaining}` : "limite estourado",
					),
				),
				el(
					"div",
