...
export function avatar(user, { size = 22 } = {}) {
	...
	const node = user.hasPhoto
		? el("img", {
				class: "avatar",
				src: `/api/photo/${user.id}?v=${user.photoVersion}`,
				alt: user.name,
				title: user.name,
				width: size,
				height: size,
...
export function renderHeader(state, { me, onWeek, onLogout }) {
	...
	return el(
		...
		el(
			...
			el(
				"button",
				{ class: "seta", disabled: atStart, "aria-label": "Semana anterior", onclick: () => onWeek(-1) },
				"‹",
			),
			el("button", { class: "hoje", onclick: () => onWeek(0) }, "Hoje"),
			el(
				"button",
				{ class: "seta", disabled: atEnd, "aria-label": "Próxima semana", onclick: () => onWeek(1) },
				"›",
			),
		),
