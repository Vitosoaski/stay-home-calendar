...
import {
  weekdayFromLabel, padTime, findPlannerLayout, parsePlanner, parseGrade, parseSubjects
} from '../src/sheets.js';
...
test('coluna em branco na linha de horários não trunca o dia', () => {
  ...
  const furado = read('planner-hf4').replace(
    'Data/Hr,07:30,08:20,09:10', 'Data/Hr,07:30,,08:20,09:10');
...
test('célula inesperada na linha de horários vira erro, não corte', () => {
  const sujo = read('planner-hf4').replace(
    'Data/Hr,07:30,08:20,09:10', 'Data/Hr,07:30,LIXO,09:10');
...
test('dezembro para janeiro vira o ano', () => {
  const planner = parsePlanner(miniPlanner([
    DEZEMBRO,
    ',,2,04/01,BD2,BD2,3,05/01,BD2,BD2,4,06/01,BD2,BD2'
  ]));
...
test('qualquer outro mês para trás é erro, não virada de ano', () => {
  assert.throws(() => parsePlanner(miniPlanner([
    DEZEMBRO,
    ',,2,04/05,BD2,BD2,3,05/05,BD2,BD2,4,06/05,BD2,BD2'
  ])), /fora de ordem/);
});
...
test('coluna extra na grade não zera as salas', () => {
  const deslocada = read('grade-4fase').split('\n').map((l) => `X,${l}`).join('\n');
