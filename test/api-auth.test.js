...
test("cadastro exige o código do grupo", async () => {
	...
	const bad = await app.post("/api/signup", { groupCode: "errado", name: "Ana", pin: "1234" });
	assert.equal(bad.status, 403);

	const good = await app.post("/api/signup", { groupCode: "segredo", name: "Ana", pin: "1234" });
...
test("cadastro recusa PIN e nome inválidos", async () => {
	...
	const longName = await app.post("/api/signup", { groupCode: "segredo", name: "x".repeat(30), pin: "1234" });
...
test("foto é guardada, servida e recusada quando grande demais", async () => {
	...
	const over = "data:image/webp;base64," + Buffer.alloc(app.config.maxPhotoBytes + 2048).toString("base64");
